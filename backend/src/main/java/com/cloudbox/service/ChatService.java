...
public class ChatService {
    ...
    private static final String LANDING_SYSTEM =
        "You are CloudBox Assistant, a helpful chatbot for the CloudBox cloud storage platform.\n" +
    ...
    private String answerFromData(String msg, UserData d) {
        ...
        if (m.contains("how many file") || m.contains("number of file") || m.contains("file count") || m.contains("total file") || m.contains("files do i"))
        ...
        if (m.contains("share") || m.contains("sharing") || m.contains("shared")) {
            String byMe = d.sharedByMe().isEmpty() ? "none"
                : d.sharedByMe().stream().limit(3).map(s -> s.getFile().getFileName() + " → " + s.getSharedWith()).collect(Collectors.joining(", "));
            String withMe = d.sharedWithMe().isEmpty() ? "none"
                : d.sharedWithMe().stream().limit(3).map(s -> s.getFile().getFileName() + " from " + s.getOwnerEmail()).collect(Collectors.joining(", "));
        ...
        return String.format("You have %d files, %.1f MB used of %d MB. Ask about files, storage, shares, folders, or activity!",
            d.totalFiles(), usedMb, d.limitMb());
    }
